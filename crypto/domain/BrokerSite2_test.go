...
var priceBroker2 float64 = 79547

func setupBrokerSite2Tests(t *testing.T, coin, coinRef, url string, forceTransportLevel bool, httpRespCode int, httpRespBody string) (func(), BrokerSite) {
	...
	if url == "" {
		server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			expectedUrl := "/v1/tools/price-conversion"
	...
	if url == "" {
		broker = NewBrokerSite2(server.Client(), server.URL)
	} else {
		broker = NewBrokerSite2(&http.Client{}, url)
	}
...
func Test_get_name_broker2(t *testing.T) {
	teardown, broker := setupBrokerSite2Tests(t, "", "", "", false, 200, "")
	defer teardown()

	name := broker.GetName()
	assert.Equal(t, "CoinMarketCap", name, fmt.Sprintf("Wrong error: got %v, want %v", name, "CoinMarketCap"))
}

func Test_get_price_broker2(t *testing.T) {
	tests := []struct {
		name                string
		url                 string
		coin                string
		...
		},
		{
			...
			forceTransportLevel: false,
			httpRespCode:        http.StatusOK,
			httpRespBody:        "{\"data\":{\"quote\":{\"ETH\":{\"price\":79547}}}}",
			expectedResponse:    nil,
			expectedError:       errs.NewUnexpectedError("Error decoding response"),
		},
		{
			...
			forceTransportLevel: false,
			httpRespCode:        http.StatusOK,
			httpRespBody:        "{\"data\":{\"quote\":{\"USD\":1}}}",
			expectedResponse:    nil,
			expectedError:       errs.NewUnexpectedError("Error decoding response"),
		},
		{
			name:                "Wrong http body - price",
			coin:                "btc",
			coinRef:             "usd",
			forceTransportLevel: false,
			httpRespCode:        http.StatusOK,
			httpRespBody:        "{\"data\":{\"quote\":{\"USD\":{\"value\":79547}}}}",
			expectedResponse:    nil,
			expectedError:       errs.NewUnexpectedError("Error decoding response"),
		},
		{
			name:                "Wrong http body - price invalid value",
			coin:                "btc",
			coinRef:             "usd",
			forceTransportLevel: false,
			httpRespCode:        http.StatusOK,
			httpRespBody:        "{\"data\":{\"quote\":{\"USD\":{\"price\":\"value\"}}}}",
			expectedResponse:    nil,
			expectedError:       errs.NewUnexpectedError("Error decoding response"),
		},
		{
			...
			forceTransportLevel: false,
			httpRespCode:        http.StatusOK,
			httpRespBody:        "{\"data\":{\"quote\":{\"USD\":{\"price\":79547}}}}",
			expectedResponse:    &priceBroker2,
			expectedError:       nil,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			teardown, broker := setupBrokerSite2Tests(t, tt.coin, tt.coinRef, tt.url, tt.forceTransportLevel, tt.httpRespCode, tt.httpRespBody)
			defer teardown()
